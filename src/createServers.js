...
function createServers({ app, level15, config, logger }) {
    ...
    http2Server.on("stream", (stream, headers) => {
        try {
            ...
            if (requestPath !== config.http2TargetPath && requestPath !== "/api/analytics.js") {
    ...
    function start() {
        ...
        http2Server.listen(config.http2Port, () => {
            logger.info(`HTTP/2 server is running at https://localhost:${config.http2Port}${config.http2TargetPath}`);
        });
    }
