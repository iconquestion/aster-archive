...
const { createTestApp, getDailyPassword, getTestRuntime } = require("../helpers/createTestApp");
...
describe("Levels 01-14", () => {
    ...
    test("08 exposes both the restricted stack path and the 09 clue", async () => {
        ...
        const noteRes = await request(app).get("/08-c2x8m5q9nv/stack/restricted/914/2013-12-31.txt");
    ...
    test("13 exposes the hidden draft page through sitemap.xml", async () => {
        ...
        const draftRes = await request(app).get("/13-k9c3x6n2tw/gallery/__draft__k9a2.html");
...
describe("Levels 16-25", () => {
    ...
    test("18 reconstructs the hidden text from multiple range chunks", async () => {
        ...
        for (let start = 80; start <= 143; start += 16) {
            ...
            expect(res.headers["content-range"]).toBe(`bytes ${start}-${expectedEnd}/138`);
