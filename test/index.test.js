...
describe("registerShutdownHandlers", () => {
    ...
    test("logs fatal rejection and exits 1 after shutdown", async () => {
        ...
        expect(logger.error).toHaveBeenCalledWith(`unhandledRejection: ${reason.stack}`);
        expect(servers.stop).toHaveBeenCalledTimes(1);
    ...
    test("exits 1 if graceful shutdown itself fails", async () => {
        ...
        expect(logger.error).toHaveBeenCalledWith(expect.stringContaining("graceful shutdown failed: Error: close failed"));
        expect(exit).toHaveBeenCalledWith(1);
    });
});
